...
This chart's bootstrap expects a `trails.zip` that contains all four networks' CSVs (including `skitouren`), so tag a release with the updated `Preprocess` workflow before installing or upgrading the chart. The import runs once per data volume: to pick up a newly added network in an existing deployment, delete the PVC (and with it the old database) so the statefulset re-bootstraps.
