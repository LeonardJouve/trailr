...
The chart downloads `https://github.com/LeonardJouve/trailr/releases/latest/download/trails.zip`, extracts the wanderwege, veloland, wanderland, mtbland and skitouren node and relationship CSV files, and runs `neo4j-admin database import full` before Neo4j starts. It records successful completion on the Neo4j data volume, so replacement pods do not repeat the import.
...
This chart's bootstrap expects a `trails.zip` that contains all five networks' CSVs, so tag a release with the updated `Preprocess` workflow before installing or upgrading the chart. The import runs once per data volume: to pick up a newly added network in an existing deployment, delete the PVC (and with it the old database) so the statefulset re-bootstraps.
